...
// src/index.ts
import { appendFileSync, existsSync as existsSync2, readFileSync as readFileSync2 } from "node:fs";
...
function toDependenciesList(value) {
...
function findNodeByName(lockfile, name) {
...
function transitiveCount(lockfile, name) {
  if (!lockfile) return null;
  const node = findNodeByName(lockfile, name);
  if (!node) return null;
  return countReachablePackages(lockfile, node);
}
function packageMetadata(lockfile, name) {
...
function fetchJson(url, init = {}, timeoutMs = 5e3) {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), timeoutMs);
  return fetch(url, { ...init, signal: controller.signal }).then(async (response) => {
...
function resolveVersionEntry(registry, packageName, version) {
...
var NpmRegistryMetadataProvider = class {
  ...
  async getPackageMetadata(packageName, version, ecosystem = "npm") {
    ...
    const versionEntry = resolveVersionEntry(registry, packageName, version);
    if (!versionEntry) {
      this.cache.set(cacheKey, null);
      return null;
    }
...
var OsvVulnerabilityProvider = class {
  ...
  timeoutMs;
  cache = new MemoryCache();
  async getVulnerabilities(packageName, version, ecosystem = "npm") {
    const cacheKey = `${ecosystem}:${packageName}:${version ?? "latest"}`;
    const cached = this.cache.get(cacheKey);
    if (cached !== void 0) return cached;
    const body = {
    ...
    const normalized = vulns.map((entry) => {
      ...
      return {
        ...
        severity: normalizeSeverity(Array.isArray(vuln["severity"]) && vuln["severity"][0] && typeof vuln["severity"][0] === "object" ? vuln["severity"][0]["type"] : vuln["severity"]),
        aliases,
        references,
        packageName,
...
async function analyzeRepository(input) {
  ...
  const policy = input.policyPath && existsSync(input.policyPath) ? parsePolicyYaml(readFileSync(input.policyPath, "utf8")) : defaultPolicyConfig;
  ...
  const result = {
    ...
    generatedAt: (/* @__PURE__ */ new Date("2026-04-19T00:00:00.000Z")).toISOString(),
...
function parseGitHubEvent() {
  ...
  const parsed = JSON.parse(readFileSync2(eventPath, "utf8"));
  return parsed;
}
...
function readActionInputs(partial = {}) {
...
function writeStdout(format, json, markdown) {
  if (format === "json") {
...
var isDirectExecution = typeof process.argv[1] === "string" && process.argv[1].endsWith("/dist/index.js");
